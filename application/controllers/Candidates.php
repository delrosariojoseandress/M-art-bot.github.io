<?php 
	...
	class Candidates extends CI_Controller
	{
		...
		public function index($id = null){
...

			{
				if($id)
				{
					...
					$this->load->view('administrator/panel/candidates', $data);
	    ...
	    {
	        $list = $this->Candidates_model->get_datatables($id);
	        $data = array();
			$no = $_POST['start'];
	        ...
	        foreach ($list as $candidates) {
	            ...
	            $row[] = '<a class="btn btn-sm btn-warning" href="javascript:void(0)" onclick="rate_to('.$candidates->applicant_id.')" title="Activity">Activity <i class="fa fa-edit"></i> </a> |
	            		...
	            		<a class="btn btn-sm btn-warning" href="'.base_url().'administrator-candidates-basic-information/'.$candidates->applicant_id.'" title="More Details">More Details <i class="fa fa-eye"></i> </a> | 
	        ...
	        $output = array(
	                        "draw" => $_POST['draw'],
	                        "recordsTotal" => $this->Candidates_model->count_all($id),
	                        "recordsFiltered" => $this->Candidates_model->count_filtered($id),
	                        "data" => $data,
                );
